 ...
 */
public class JobAdConverter implements JsonDeserializer<JobAd> {

    @Override
    public JobAd deserialize(JsonElement je, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        if (je.getAsJsonObject().get("type") != null && JobAd.TYPE.equals(je.getAsJsonObject().get("type").getAsString())) {
            Iterator<Entry<String, JsonElement>> it = je.getAsJsonObject().entrySet().iterator();
            JobAd company = new JobAd();
